<template>
	<view class="register">
		<view class="title-wrapper">
			...
			<text class="exam-title">手机号</text>
		</view>
		<view class="form-group">
			...
			<view class="form-group-item">
				<view class="form-group-item-input">
					<input class="input-content" placeholder="请输入" placeholder-style="color:#DDDDDD" v-model="verify" />
					<view class="send-btn" @click="sendSms()">
						<text>{{sendText}}</text>
					</view>
				</view>
				<view class="form-submit">
					<view class="submit-btn" @click="toPassword()">
						<text class="submit-btn-text">下一步</text>
					</view>
				</view>
			</view>
		</view>
		...
		methods: {
			toPassword() {
				if (!this.verify) {
					...
					uni.navigateTo({
						url: `./password?phone=${this.phone}&username=${this.nickname}&verify=${this.verify}`
					})
				}
			},
			back() {
